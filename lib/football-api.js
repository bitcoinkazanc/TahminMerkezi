...
/*
 * Türkçe karakterleri URL slug'ına
 * uygun hale getirir.
 */
function slugify(
  value = ""
 ...
 *
 * Örnek:
 * /mac/besiktas-vs-galatasaray/iddaa/MAÇ_ID
 */
...
/*
 * Bir HTML elementindeki bütün text'i
 * temiz şekilde alır.
 */
function cleanText(
  value = ""
...
/*
 * Market içindeki seçimleri bulmaya çalışır.
 *
 * Mackolik'in HTML yapısı zaman içinde
 * değişebileceği için birden fazla selector
 * kullanıyoruz.
 */
function parseMarketSelections(
  ...
  /*
   * Aynı seçim birden fazla selector
   * tarafından yakalanmış olabilir.
   */
  const unique =
    [];
...
/*
 * Mackolik İddaa market HTML'sini
 * normalize eder.
 */
function parseIddaaMarkets(
  ...
  const marketElements =
    $(
      [
        ".widget-iddaa-markets__markets-list > li",
        ".widget-iddaa-markets__market",
        "[class*='widget-iddaa-markets__market']"
      ].join(",")
    ).toArray();
  ...
  /*
   * Bazı sayfalarda üst selector
   * bütün market alanını yakalayabilir.
   * Aynı marketleri temizliyoruz.
   */
  const uniqueMarkets =
    [];
...
/*
 * Mackolik maçının İddaa sayfasını getirir
 * ve gerçek marketleri parse eder.
 */
...
export async function getMatches() {
  ...
  /*
   * Burada herhangi bir limit yok.
   *
   * Mackolik kaç maç döndürürse
   * tamamını geri gönderiyoruz.
   */
  return [
    ...footballMatches,
    ...basketballMatches,
