...
function calculateMinuteFromTimestamp(
  ...
  /*
   * periodStart yoksa mstUtc
   * kullanılabilir.
   *
   * Ancak yalnızca normal canlı
   * maçlarda kullanılacak.
   */
  if (
    start === null
  ) {
...
function calculateLiveMinute(
  ...
  /*
   * Sadece gerçekten live olan
   * maçta dakika hesapla.
   */
  if (
    getState(match) !==
    "live"
   ...
   *
   * ÖNEMLİ:
   * Mackolik bazen burada
   * 129 gibi bir değer döndürüyor.
   * Bu normal maç dakikası değildir.
   */
  if (
    isPenaltyShootout(
      match
  ...
  /*
   * Önce Mackolik'in gerçek
   * dakika alanlarını kullan.
   */
  const direct =
    getDirectLiveMinute(
      match
...
function normalizeStatus(
   ...
   * SIRALAMA ÖNEMLİ.
   *
   * Önce finished kontrol edilir.
   * Böylece state/live gibi daha genel
   * alanlar finished durumunu ezemez.
   */
  if (
    isFinished(match)
  ) {
...
function normalizeMatch(
  ...
  return {
    ...
    /*
     * Penaltı atış skorlarını
     * normal maç skorundan ayrı
     * tutuyoruz.
     */
    penalty_home_score:
      getScoreValue(
